...
public class UserLayerDbServiceMybatisImpl extends UserLayerDbService {
    ...
    public UserLayer getUserLayerById(long id) {
        ...
        try {
            final UserLayerMapper mapper = session.getMapper(UserLayerMapper.class);
            UserLayer userLayer = mapper.findUserLayer(id);
    ...
    public List<UserLayer> getUserLayerByUuid(String uuid) {
        ...
        try {
            ...
            for(UserLayer userLayer : userLayers) {
                String bounds = getUserLayerBounds(userLayer.getId());
                userLayer.setBounds(bounds);
            }
    ...
    public String getUserLayerBounds(long id) {
        ...
        try {
            final UserLayerMapper mapper = session.getMapper(UserLayerMapper.class);
            return mapper.calculateUserLayerBounds(id);
        } catch (Exception e) {
            log.error(e, "Failed to get bounds for userLayer with id:", id);
        } finally {
            session.close();
        }
        return null;
