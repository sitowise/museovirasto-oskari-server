...
public class V1_39__add_nba_registers_to_published_view_where_missing implements JdbcMigration {

    private static final Logger LOG  = LogFactory.getLogger(V1_39__add_nba_registers_to_published_view_where_missing.class);
    ...
    public void migrate(Connection connection) throws Exception {
        ...
        List<Long> viewsToUpdate = getPublishedViewsWithoutNbaRegistersBundle(connection);
        long toolbarId = getIdForNbaRegistersBundle(connection);
    ...
    private List<Long> getPublishedViewsWithoutNbaRegistersBundle(Connection conn) throws SQLException {
        ...
        String sql = "select id from portti_view where (type = 'PUBLISHED' OR type = 'PUBLISH') and id NOT IN (select view_id from portti_view_bundle_seq\n" +
                ...
                " and bundle_id = (select id from portti_bundle where name = 'nba-registers'))";
    ...
    private Long getIdForNbaRegistersBundle(Connection conn) throws Exception {
        String sql = "select id from portti_bundle where name = 'nba-registers'";
        try(PreparedStatement statement = conn.prepareStatement(sql)) {
            try (ResultSet rs = statement.executeQuery()) {
                if(rs.next()) {
        ...
        throw new Exception("nba-registers bundle not in portti_bundle");
    }
